...
function SingleCountry({ filtered }) {
	const [weather, setWeather] = useState([]);
	const api_key = import.meta.env.VITE_SOME_KEY

	useEffect(() => {
		axios(
			`http://api.openweathermap.org/geo/1.0/direct?q=${filtered[0].capital}&appid=${api_key}`
		)
			.then((response) => {
				...
				return axios(
					`http://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=metric&exclude=minutely,hourly,alerts&appid=${api_key}`
				);
			})
			.then((response) => {
	...
	return (
		<>
			...
			<div>Capital City: {filtered[0].capital}</div>
			<div>Area: {filtered[0].area}</div>
			<div>
				Languages:
				<ul>
					{Object.entries(filtered[0].languages).map((arr) => {
